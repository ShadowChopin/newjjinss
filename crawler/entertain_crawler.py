...
def collect_entertain_news():
  print("{} started collecting articles in entertainment".format(ci.LOG_PREFIX))

  soup = cl.get_html(ci.ENTERTAIN_URL)
  ...
  num_of_news = 0
  for class_ in ci.CLASS_ENTERTAIN:
    news_each = soup.find('div', attrs = {'data-category':class_})
    li_list = news_each.find_all('li')

    idx = 0
    for li in li_list:
      div_tag = li.find('div', attrs = {'class':'cont_thumb'})
      a_tag = div_tag.find('a')
      title = a_tag.string # 기사 타이틀 --> DB
      src_link = a_tag['href']  # url 정보 --> DB
      press = a_tag['data-tiara-provider']  # 언론사 --> DB

      soup_in = cl.get_html(src_link) # 기사 작성일을 가져오기 위해 기사 본문 액세스
      date = soup_in.find('span', attrs = {'class':'num_date'}).string
      
      id = cl.generate_id(ci.NEWS_CLASS.ENTERTAINMENT, date, str(num_of_news))
      ...
      # img_tag = li.find('img')    # thumbnail extraction
      # #urllib.request.urlretrieve(img_tag['src'], "Bigdata_analysis_textbook/portfolio/crawler/img/thumb{}{}.jpg".format(class_, idx))
      # #urllib.request.urlretrieve(img_tag['src'], "crawler/img/thumb{}{}.jpg".format(class_, idx))
      ...
      #with open("crawler/html/thumb{}{}.jpg".format(class_, idx), 'w', encoding = 'utf-8-sig') as f:
      ...
      np_srclink = np.append(np_srclink, src_link)
      ...
      #np_thumbnail = np.append(np_thumbnail, "resources/thumbnails/thumb{}.jpg".format(id))
      np_subject = np.append(np_subject, ci.NEWS_CLASS.ENTERTAINMENT)
